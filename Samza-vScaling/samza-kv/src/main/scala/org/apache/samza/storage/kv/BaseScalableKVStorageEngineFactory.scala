 ...
 */
trait BaseScalableKVStorageEngineFactory[K, V] extends StorageEngineFactory[K, V] {
  ...
  def getStorageEngine(storeName: String,
    ...
    val maybeLoggedStore = if (changeLogSystemStreamPartition == null) {
      ...
      new LoggedStore(rawStore.asInstanceOf[KeyValueStore[Array[Byte], Array[Byte]]], changeLogSystemStreamPartition, collector, loggedStoreMetrics)
    }

    // wrap with serialization
    val serializedMetrics = new SerializedKeyValueStoreMetrics(storeName, registry)
    val serialized = new SerializedKeyValueStore[K, V](maybeLoggedStore.asInstanceOf[KeyValueStore[Array[Byte], Array[Byte]]], keySerde, msgSerde, serializedMetrics)
    ...
    new ScalableKeyValueStorageEngine(storeName, storeDir, storePropertiesBuilder.build(), nullSafeStore, rawStore,
