...
import org.apache.samza.storage.{ScalableStorageEngine, StoreProperties}
 ...
 */
class ScalableKeyValueStorageEngine[K, V](
                                           storeName: String,
                                           storeDir: File,
                                           storeProperties: StoreProperties,
                                           wrapperStore: KeyValueStore[K, V],
                                           rawStore: ScalableKeyValueStore[Array[Byte], Array[Byte]],
                                           metrics: KeyValueStorageEngineMetrics = new KeyValueStorageEngineMetrics,
                                           batchSize: Int = 500,
                                           val clock: () => Long = { System.nanoTime }) extends ScalableStorageEngine with KeyValueStore[K, V] with TimerUtil with Logging {
  ...
  def restore(envelopes: java.util.Iterator[IncomingMessageEnvelope]) {
    ...
    for (envelope <- envelopes.asScala) {
      ...
      if (batch.size >= batchSize) {
        doPutAll(rawStore.asInstanceOf[KeyValueStore[Array[Byte], Array[Byte]]], batch)
        batch.clear()
      }
