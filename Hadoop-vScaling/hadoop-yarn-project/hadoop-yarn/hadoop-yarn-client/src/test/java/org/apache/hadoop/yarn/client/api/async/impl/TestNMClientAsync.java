...
import org.apache.hadoop.yarn.api.records.Resource; //DrG
  ...
  private NMClient mockNMClient(int mode)
    ...
    switch (mode) {
      case 0:
        ...
        doNothing().when(client).increaseContainerResource(//DrG
                any(Container.class));//DrG
      ...
      case 2:
        ...
        when(client.getContainerStatus(any(ContainerId.class),
            any(NodeId.class))).thenReturn(
                recordFactory.newRecordInstance(ContainerStatus.class));
        doThrow(RPCUtil.getRemoteException("Increase Resource Exception"))//DrG
                         .when(client).increaseContainerResource(any(Container.class));//DrG
        doThrow(RPCUtil.getRemoteException("Stop Exception")).when(client)
            .stopContainer(any(ContainerId.class), any(NodeId.class));
    }
    return client;
