...
public class CGroupsMemoryResourceHandlerImpl implements MemoryResourceHandler {
  ...
  public List<PrivilegedOperation> updateContainerResource(ContainerId containerId,
    ...
    //DrG
    LOG.info("Update the container's memory " + containerId.toString() + "to the target resource " + containerResource.toString());
  ...
  private void setupLimits(String cgroupId,
    ...
    long containerSoftLimit =
            (long) (containerResource.getMemory() * this.softLimit);
    long containerHardLimit = containerResource.getMemory();
    try {
      cGroupsHandler.updateCGroupParam(MEMORY, cgroupId,
              CGroupsHandler.CGROUP_PARAM_MEMORY_HARD_LIMIT_BYTES,
              String.valueOf(containerHardLimit) + "M");
