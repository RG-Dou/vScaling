...
import org.apache.hadoop.yarn.server.nodemanager.containermanager.linux.resources.ResourceHandlerException;//DrG-4166
      ...
      if (!serviceStopped) {
        ...
        try {
          this.context.getContainerExecutor().updateContainerResource(
                  containerId, targetResource);
        } catch (ResourceHandlerException re) { // DrG - 4166
          this.context.getNMStateStore().storeContainerResourceChanged(
                  containerId, currentResource);
          throw re;
        }
